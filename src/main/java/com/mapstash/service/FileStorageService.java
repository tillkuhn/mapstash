...
public class FileStorageService {
  ...
  public GpxFile storeFile(MultipartFile file, String description) throws IOException {
    ...
    log.debug(
        "Saving GPX content for fileId={} length={} preview={}",
        gpxFile.getId(),
        contentLen,
        preview.replaceAll("\n", "\\n"));

    // Validate that the content looks like GPX/XML and not a numeric value (previous bug stored
    // numeric values accidentally)
    String trimmed = contentString.trim();
    if (trimmed.matches("^\\d+$") || !trimmed.startsWith("<")) {
      log.error(
          "Refusing to persist invalid GPX content for fileId={} (length={}, preview={})",
          gpxFile.getId(),
          contentLen,
          preview.replaceAll("\n", "\\n"));
      throw new IllegalStateException(
          "Invalid GPX content detected; aborting save to avoid corrupting database");
    }

    gpxContent.setGpxContent(contentString);
    // Use saveAndFlush to ensure content is written immediately to DB (helps with debugging and
    // JDBC streaming)
    ...
    if (persisted.isPresent()) {
      ...
      String storedPreview =
          (stored == null) ? "" : (storedLen > 200 ? stored.substring(0, 200) + "..." : stored);
      log.debug(
          "Persisted GPX content for fileId={} storedLength={} preview={}",
          gpxFile.getId(),
          storedLen,
          storedPreview.replaceAll("\n", "\\n"));
      if (storedLen != contentLen) {
        log.warn(
            "Mismatch between original content length={} and stored length={} for fileId={}",
            contentLen,
            storedLen,
            gpxFile.getId());
      }
  ...
  public String getGeoJsonForFile(String fileId) throws IOException {
    // Prefer streaming the content via JDBC for large content. Fall back to repository-stored
    // content string.
    ...
    GpxContent content =
        contentRepository
            .findByGpxFileId(fileId)
            .orElseThrow(
                () -> new IllegalArgumentException("GPX content not found for file: " + fileId));
    try (InputStream in2 =
        new java.io.ByteArrayInputStream(
            content.getGpxContent().getBytes(java.nio.charset.StandardCharsets.UTF_8))) {
